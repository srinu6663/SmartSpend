...
const WalletCard = ({ w, i, totalBalance }: { w: Wallet; i: number; totalBalance: number }) => {
  ...
  return (
    ...
    >
      <div
        className="relative w-full h-[108px] rounded-2xl p-4 overflow-hidden shadow-lg flex flex-col justify-between"
      ...
      >
        ...
        <div className="flex items-center justify-between z-10">
          ...
          <div className="flex items-center gap-1">
            <span className="text-[9px] font-black uppercase tracking-[0.1em] text-white/70 bg-black/15 px-2 py-0.5 rounded-full">
              {typeLabel}
            </span>
          </div>
        </div>

        {/* Bottom: name + amount + share bar */}
        <div className="z-10 space-y-1">
          <div className="flex items-end justify-between">
            <div>
              <p className="text-[10px] font-semibold text-white/75 truncate mb-0.5 max-w-[90px]">{w.name}</p>
              <p className="text-[17px] font-black tabular-nums text-white tracking-tight leading-none">
                ₹{Number(w.balance).toLocaleString("en-IN", { minimumFractionDigits: 0 })}
              </p>
            </div>
            <span className="text-[11px] font-bold text-white/60 pb-0.5">{pct}%</span>
          </div>
          {/* Mini share bar */}
    ...
    <>
      <div className="px-4">
          ...
          <div>
            ...
            {wallets.length > 1 && (
              <p className="text-[10px] text-muted-foreground mt-0.5">
                Combined ₹{totalBalance.toLocaleString("en-IN", { minimumFractionDigits: 0 })}
              </p>
            )}
          </div>
          <button
        ...
        <div className="flex gap-3 overflow-x-auto no-scrollbar pb-3 -mx-4 px-4 snap-x snap-mandatory">
          {wallets.length === 0 ? (
            <div className="w-[175px] h-[108px] rounded-2xl bg-muted/60 border border-border/40 flex flex-col items-center justify-center shrink-0 snap-start">
              ...
              <span className="text-[10px] text-muted-foreground/35 mt-0.5">Tap Add to begin</span>
            </div>
          ) : (
            wallets.map((w, i) => <WalletCard key={w.id} w={w} i={i} totalBalance={totalBalance} />)
          )}
        </div>
      </div>
